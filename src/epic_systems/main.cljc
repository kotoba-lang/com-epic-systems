(ns epic-systems.main
  "Kotodama WASM entrypoint for the Epic-systems clean-room actor (L5) — Clojure port.
  ...
  facts (`epic_systems.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "epic_systems")
(def tier "L5")
...
(def entity-specs
  [{:entity "Patient"   :plural "patients"   :id-prefix "epicsyst_pat"
    :fields [:resourceType :implicitRules :language :active :gender :birthDate :deceasedBoolean :deceasedDateTime :multipleBirthBoolean :multipleBirthInteger]
    :required [:resourceType :implicitRules]
    :coerce {:active :bool :deceasedBoolean :bool :multipleBirthBoolean :bool :multipleBirthInteger :int}
    :refs {}}
   {:entity "Observation"   :plural "observations"   :id-prefix "epicsyst_obs"
    :fields [:resourceType :implicitRules :language :status :issued]
    :required [:resourceType :implicitRules]
    :coerce {}
    :refs {}}
   {:entity "Encounter"   :plural "encounters"   :id-prefix "epicsyst_enc"
    :fields [:resourceType :implicitRules :language :status]
    :required [:resourceType :implicitRules]
    :coerce {}
    :refs {}}
   {:entity "Condition"   :plural "conditions"   :id-prefix "epicsyst_con"
    :fields [:resourceType :implicitRules :language :recordedDate]
    :required [:resourceType :implicitRules]
    :coerce {}
    :refs {}}
   {:entity "MedicationRequest"   :plural "medicationrequests"   :id-prefix "epicsyst_med"
    :fields [:resourceType :implicitRules :language :status :intent :priority :doNotPerform :reportedBoolean :authoredOn :instantiatesUri]
    :required [:resourceType :implicitRules]
    :coerce {:doNotPerform :bool :reportedBoolean :bool}
    :refs {}}
   {:entity "Procedure"   :plural "procedures"   :id-prefix "epicsyst_pro"
    :fields [:resourceType :implicitRules :language :instantiatesUri :status]
    :required [:resourceType :implicitRules]
    :coerce {}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"epic_systems.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "epic-systems-compat" :tier tier :entities entities} 200])
...
(defn start! [] :epic-systems-compat/ready)
