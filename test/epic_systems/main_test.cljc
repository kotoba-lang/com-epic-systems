(ns epic-systems.main-test
  "Contract + behavioral test for the epic-systems-compat L5 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "epic-systems-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'epic-systems.main-test)]
